let settings = { mode: 'ai', difficulty: 'easy', diceSort: 'value', alwaysFirst: false, theme: 'dark', luckYou: 0, luckOpp: 0 }; let state = null;
function rollDie(s, player) {
    // luck: -3 to +3. Positive = bias toward higher rolls
    const L = (player === 1 ? settings.luckYou : settings.luckOpp) || 0;
...
function createFreshState() {
    const s = {
        currentPlayer: 1, firstPlayer: 1, attackType: 'strength', selectedDice: [], targetDie: null, expression: [],
        youDice: DICE_CONFIG.map((d, i) => ({ id: `you_${i}`, sides: d.sides, name: d.name, value: rollDie(d.sides, 1), captured: false })),
        opponentDice: DICE_CONFIG.map((d, i) => ({ id: `opp_${i}`, sides: d.sides, name: d.name, value: rollDie(d.sides, 2), captured: false })),
